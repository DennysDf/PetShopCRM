                                ...
                                IUserService userService) : Controller
{
    public async Task<IActionResult> Index(int id = 0)
    {
        ...
        if (id != 0)
        {
            var recordDTO = await recordService.GetByIdAsync(id);
            if (recordDTO.Success)
            {
                var record = recordDTO.Data;
                recordVM = recordVM.ToVM(record);
                var plan =  paymentService.GetPlanByPet(recordVM.PetId);
                recordVM.HealthPlanId = plan.Id;
                ViewData["Procedures"] = await GetProcedureByPet(recordVM.PetId);
            }
        }
        else
        {
            recordVM.Date = DateTime.Now.ToDateBrazilMin().ToDateTime();
        ...
        recordVM.ListPets = new SelectList(pets.Select(c => new { c.Id, Name = $"{c.Name} - {c.Identifier} - {c.Guardian.Name}" }).ToList(), nameof(Pet.Id), nameof(Pet.Name));
        recordVM.ListHealthPlan = new SelectList(healthPlan.Select(c => new { c.Id, Name = $"{c.Name} - R$ {c.Value}" }).ToList(), nameof(HealthPlan.Id), nameof(HealthPlan.Name));
        recordVM.ListClinics = new SelectList(clinics.Select(c => new { c.Id, Name = $"{c.Name}" }).ToList(), nameof(Clinic.Id), nameof(Clinic.Name));
